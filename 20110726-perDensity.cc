...
static Ptr<Vehicle> CreateObstacle(Ptr<Highway> highway, int lane, int direction, int position)
{
	...
	if(direction==1)
		vhc->SetPosition(Vector(position,highway->GetYForLane(0,1),0));
	else if(direction==-1)
		vhc->SetPosition(Vector(position,highway->GetYForLane(1,-1),0));
	...
	vhc->SetAcceleration(0.0);
	Ptr<Model> vhcModel=highway->CreateSedanModel();
	vhcModel->SetDesiredVelocity(0.0);  // max speed 36(m/s)
	vhc->SetModel(vhcModel);          // or common sedan model: highway->GetSedanModel()
	vhc->SetLength(4);
	vhc->SetWidth(2);
...
//	Simulator::Schedule(Seconds(10.0), &VehicleBreak, vehW1);
	...
	// Add an obstacle at 5000m
	Ptr<Vehicle> obsW1=CreateObstacle(highway, 1, 1, 10000);
	AddVehicle(highway,obsW1);

	// Schedule 'obstacle' message on a given time
	// Obstacle is invisible until that point, cars just go over it now
	// we'll start with t=200
	unsigned int pID=1;
	Simulator::Schedule(Seconds(200.0), &AddPacket, obsW1, pID);

	/*
	 * Schedule Destination vehicle, 10km away from the obstacle
	 */
	Ptr<Vehicle> vehDst=CreateVehicle(highway, 1, 1);
	vehDst->SetSilence(true);	// Simulation will end when packet reaches this vehicle
	Simulator::Schedule(Seconds(200.0), &AddVehicle, highway, vehDst);
...
//	Simulator::Schedule(Seconds(14.2), &AddVehicle, highway, vehW2);
	...
	if(nLanes==2) ExponentialAddVehicles(highway, 2, -1);
...
//	packet->CopyData (&data, sizeof(unsigned int));
	...
	if(isNew==true)
	{
		...
		/*
		 * Vehicle breaks on message reception.
		 */
		if(veh->GetDirection()==1) // don't break opposite-lane vehicles
		{
			if(doBrake) veh->SetAcceleration(-5.0);
...
//	double transmissionPower=18.9; // not enough
	...
	cmd.AddValue ("density", "density", cliDensity);
