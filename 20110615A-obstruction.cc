...
static bool InitVehicle(Ptr<Highway> highway, int& VID)
{
	 ...
	 */
	/*
	 * Add an obstacle. See if vehicle stops behind it. Verify.
	 */
...
//	Simulator::Schedule(Seconds(10.0), &VehicleBreak, vehW1);
	...
	Ptr<Vehicle> obsW1=CreateObstacle(highway, 1, 1, 5000);
	{	// Obstruction broadcasts message, packet 1, after 1 second
		unsigned int pID=1;
		Simulator::Schedule(Seconds(1), &AddPacket, obsW1, pID);
	}
...
{ 
	...
	// Defaults
	float simTime=320.0;		// 200s to fill 6km road
	int runNumber=1;
